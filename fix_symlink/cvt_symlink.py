...
def make_relative_target_force(root_dir: str, link_path: str, abs_target: str) -> str | None:
    """
    Convert an absolute target like '/usr/lib/libc.so'
    into a relative path from directory of link_path, even if target doesn't exist.

    Returns None if the resolved target would not be inside root_dir.
    """
    root_dir = os.path.abspath(root_dir)

    # Resolved absolute path within extracted root (whether it exists or not)
    candidate = os.path.normpath(os.path.join(root_dir, abs_target.lstrip("/")))

    # Ensure candidate would be inside root_dir
    if not candidate.startswith(root_dir + os.sep) and candidate != root_dir:
        return None

    # Even if candidate doesn't exist, we can still calculate the relative path
...
def fix_symlink_force(root_dir: str, link_path: str, dry_run: bool, verbose: bool) -> None:
    try:
    ...
    new_target = make_relative_target_force(root_dir, link_path, target)
    if new_target is None:
        if verbose:
            print(f"[INFO] Skipping {link_path} -> {target} (target would be outside root)")
...
def walk_and_fix_force(root_dir: str, dry_run: bool, verbose: bool) -> None:
    ...
    for cur_dir, dirs, files in os.walk(root_dir, followlinks=False):
        ...
        for name in dirs + files:
            ...
            if os.path.islink(path):
                ...
                fix_symlink_force(root_dir, path, dry_run=dry_run, verbose=verbose)

def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Fix symlinks in an extracted root by converting absolute targets "
            "to relative paths (even if the target does not exist under the root)."
...
def main():
    args = parse_args()
    try:
        walk_and_fix_force(args.root_dir, dry_run=args.dry_run, verbose=args.verbose)
